...
def relax_protocol(inlet, protocol_type, ENG, relax_time = 10, start = True):
    ...
    if start:
        if protocol_type == 'control':
            ...
            if ENG:
                move_command = "py -2 nao_controls.py --mode 1 --sound inicio --english True"  # launch python2 script
                process = subprocess.Popen(move_command.split(), stdout=subprocess.PIPE, text = True)
            else:
                move_command = "py -2 nao_controls.py --mode 1 --sound inicio" 
                process = subprocess.Popen(move_command.split(), stdout=subprocess.PIPE, text = True)
            time.sleep(24)
        else:
            time.sleep(23)
    ...
    if start:
        ...
        if protocol_type == 'control':
            ...
            if ENG:
                move_command = "py -2 nao_controls.py --mode 1 --sound acomenzar --english True"  # launch python2 script
                process = subprocess.Popen(move_command.split(), stdout=subprocess.PIPE, text = True)
            else:
                move_command = "py -2 nao_controls.py --mode 1 --sound acomenzar"  
                process = subprocess.Popen(move_command.split(), stdout=subprocess.PIPE, text = True)
            time.sleep(6)
        else:
            time.sleep(6) 
...
def robot_protocol(mov, ENG):
    ...
    if mov == 'right':
        move_command = "py -2 nao_controls.py --mode 2 --move right"  # launch python2 script
        process = subprocess.Popen(move_command.split(), stdout=subprocess.PIPE, text = True)
    elif mov == 'left':
        move_command = "py -2 nao_controls.py --mode 2 --move left"  
        process = subprocess.Popen(move_command.split(), stdout=subprocess.PIPE, text = True)
    else:
        move_command = "py -2 nao_controls.py --mode 2 --move both" 
        process = subprocess.Popen(move_command.split(), stdout=subprocess.PIPE, text = True)
