...
norm_data.FC1 = struct('thetaPSD', zeros(nrows,10), 'alphaPSD', zeros(nrows,10), 'betaPSD', zeros(nrows,10));
norm_data.FCz = struct('thetaPSD', zeros(nrows,10), 'alphaPSD', zeros(nrows,10), 'betaPSD', zeros(nrows,10));
norm_data.FC2 = struct('thetaPSD', zeros(nrows,10), 'alphaPSD', zeros(nrows,10), 'betaPSD', zeros(nrows,10));
norm_data.C3 = struct('thetaPSD', zeros(nrows,10), 'alphaPSD', zeros(nrows,10), 'betaPSD', zeros(nrows,10));
norm_data.C4 = struct('thetaPSD', zeros(nrows,10), 'alphaPSD', zeros(nrows,10), 'betaPSD', zeros(nrows,10));
norm_data.CP1 = struct('thetaPSD', zeros(nrows,10), 'alphaPSD', zeros(nrows,10), 'betaPSD', zeros(nrows,10));
norm_data.CPz = struct('thetaPSD', zeros(nrows,10), 'alphaPSD', zeros(nrows,10), 'betaPSD', zeros(nrows,10));
norm_data.CP2 = struct('thetaPSD', zeros(nrows,10), 'alphaPSD', zeros(nrows,10), 'betaPSD', zeros(nrows,10));
...
for i = 1:length(dataChs)
    ...
    for j = 1:length(WaveNames)
        ...
        averages_norm(row_counter,:) = mean(norm_matrix(:,2:10));
        ...
        T_extended_norm = array2table(norm_matrix, 'VariableNames',{'Participants','Video_Right','Video_Left', 'Video_Both', 'Robot_Right','Robot_Left', 'Robot_Both', 'VR_Right', 'VR_Left', 'VR_Both'});
        T_extended_norm.Participants = extendedParticipants_t;
        
        % Define filename: SubStructName_MatrixName.csv
        filename = sprintf('%s_%s.csv', ChName, WName);
        fullPath = fullfile(bp_folder, filename);

        filename_norm = sprintf('%s_%s_norm.csv', ChName, WName);
...
T_averages_norm = array2table(averages_norm, 'VariableNames',{'Video_Right','Video_Left', 'Video_Both', 'Robot_Right','Robot_Left', 'Robot_Both', 'VR_Right', 'VR_Left', 'VR_Both'}, ...
    'RowNames', {'FC1_theta','FC1_alpha','FC1_beta','FCz_theta','FCz_alpha','FCz_beta','FC2_theta','FC2_alpha','FC2_beta','C3_theta','C3_alpha','C3_beta','C4_theta','C4_alpha','C4_beta', ...
    'CP1_theta','CP1_alpha','CP1_beta','CPz_theta','CPz_alpha','CPz_beta','CP2_theta','CP2_alpha','CP2_beta'});
