    ...
    for rep in range(len(movements_list)):
        ...
        if rep == (n_rep -1):
            if protocol_type == 'control':
                ...
                if ENG:
                    move_command = "py -2 nao_controls.py --mode 1 --sound fin --english True"  # launch python2 script
                    process = subprocess.Popen(move_command.split(), stdout=subprocess.PIPE, text = True)
                else:
                    move_command = "py -2 nao_controls.py --mode 1 --sound fin"  # launch python2 script
                    process = subprocess.Popen(move_command.split(), stdout=subprocess.PIPE, text = True)
            else:
                pass
